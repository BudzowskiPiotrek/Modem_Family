...
/**
 * Frontend panel for CRUD (Create, Read, Update, Delete) operations. Provides a
 * dynamic interface to interact with different database tables, including a
 * sidebar for table selection, a central data table, and a bottom form for data
 * entry and editing.
 */
public class CrudFrontend extends JPanel {
	...
	public void applyTheme() {
		...
		if (scrollTablasPanel != null) {
			scrollTablasPanel.setBorder(BorderFactory.createTitledBorder(new LineBorder(border), Language.get(71),
					javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.TOP, FUENTE_TITULO, txt));
		...
		if (scrollTablaPanel != null) {
			...
			scrollTablaPanel.setBorder(BorderFactory.createTitledBorder(new LineBorder(border), Language.get(72),
					javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.TOP, FUENTE_TITULO, txt));
		}

		if (panelSur != null) {
			panelSur.setBackground(bgPanel);
			panelSur.setBorder(new CompoundBorder(new LineBorder(border), new EmptyBorder(15, 15, 15, 15)));
		}
				...
				if (comp instanceof JPanel) {
					...
					for (Component subComp : campoPanel.getComponents()) {
						if (subComp instanceof JLabel) {
							...
							field.setBorder(new CompoundBorder(new LineBorder(border), new EmptyBorder(5, 8, 5, 8)));
						}
					}
				}
			}
		...
		if (btnGuardar != null) {
			...
			btnGuardar.setBorder(new CompoundBorder(new LineBorder(accent, 1), new EmptyBorder(8, 20, 8, 20)));
		}

		if (btnVolver != null) {
			btnVolver.setBackground(bgPanel);
			btnVolver.setForeground(txt);
			btnVolver.setBorder(new CompoundBorder(new LineBorder(border, 1), new EmptyBorder(8, 20, 8, 20)));
		}

		if (btnCancelarEdicion != null) {
			btnCancelarEdicion.setBackground(danger);
			btnCancelarEdicion.setForeground(Color.WHITE);
			btnCancelarEdicion.setBorder(new CompoundBorder(new LineBorder(danger, 1), new EmptyBorder(8, 20, 8, 20)));
		}
	...
	/**
	 * Updates all localized texts and titles based on the current language
	 * selection.
	 */
	public void updateAllTexts() {
		scrollTablasPanel.setBorder(BorderFactory.createTitledBorder(new LineBorder(Common.getBorder()),
				Language.get(71), javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.TOP,
				FUENTE_TITULO, Common.getText()));

		scrollTablaPanel.setBorder(BorderFactory.createTitledBorder(new LineBorder(Common.getBorder()),
				Language.get(72), javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.TOP,
				FUENTE_TITULO, Common.getText()));
	...
	/**
	 * Retrieves the current column names and updates the action column header
	 * localized string.
	 * 
	...
	/**
	 * Initializes the bottom panel containing labels, input forms, and action
	 * buttons.
	 */
	...
	/**
	 * Injects an 'Actions' column into the data table containing Edit and Delete
	 * buttons.
	 */
	...
	/**
	 * Dynamically generates the input form based on the column names of the
	 * selected table.
	 * 
	 * @param nombresColumnas Array of column names to generate labels and text
	 *                        fields.
	 */
	public void generarFormulario(String[] nombresColumnas) {
		...
		for (String nombreCol : nombresColumnas) {
			...
			textField.setBorder(new CompoundBorder(new LineBorder(Common.getBorder()), new EmptyBorder(5, 8, 5, 8)));
	...
	/**
	 * Clears all text fields in the data entry form and resets labels to 'Create'
	 * mode.
	 */
	...
	/**
	 * Logic to determine if the current user has permissions to modify
	 * (Edit/Delete) the currently selected table.
	 * 
